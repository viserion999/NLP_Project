...
Existing metrics (BLEU, ROUGE) measure lexical overlap but fail to capture lyrical essence: phonetic flow, emotional continuity, rhythmic structure, hook catchiness, and semantic coherence. We introduce \textbf{PHREM} (Phonetic Rhythm Emotion Metric), a composite evaluation framework designed specifically for emotion-conditioned lyric quality assessment.

PHREM integrates seven complementary dimensions, each capturing distinct aspects of lyric quality. The metric is computed as a weighted sum:
\begin{equation}
\text{PHREM} = \sum_{i=1}^{7} \alpha_i c_i
\end{equation}
where weights are: $\alpha_{\text{ppfs}}=0.12$, $\alpha_{\text{rsfs}}=0.13$, $\alpha_{\text{eas}}=0.22$, $\alpha_{\text{hqcs}}=0.13$, $\alpha_{\text{rcs}}=0.10$, $\alpha_{\text{mcs}}=0.20$, $\alpha_{\text{dps}}=0.10$. All components are normalized to $[0,1]$, ensuring $\text{PHREM} \in [0,1]$.

\paragraph{PPFS: Phonetic Pattern Flow Score.}
PPFS evaluates the smoothness and naturalness of lyric delivery at the phoneme level. High-quality lyrics exhibit smooth transitions between phonemes without jarring vowel-consonant shifts or stress disruptions. For adjacent phonemes $p_i$ and $p_{i+1}$:

\begin{equation}
c(p_i, p_{i+1}) = 0.6 \cdot \mathbb{1}[\text{v}(p_i) \neq \text{v}(p_{i+1})] + 0.4 \cdot |s_i - s_{i+1}|
\end{equation}

where $\mathbb{1}[\cdot]$ indicates vowel-consonant transition (penalty 0.6), and $|s_i - s_{i+1}|$ captures stress mismatch (weight 0.4). The final score is $\text{PPFS} = \exp(-\bar{c})$, where $\bar{c}$ is average transition cost. This ensures smooth phonetic flow scores near 1.0 for natural-sounding lyrics.

\paragraph{RSFS: Rhythmic Structure Flow Score.}
RSFS measures rhythmic consistency by evaluating syllable uniformity and stress pattern regularity across lyric lines. Songs with consistent syllable counts and predictable stress patterns are easier to sing and more memorable. Syllable consistency relative to median line length:

\begin{equation}
R_s = 1 - \frac{1}{n} \sum_{i=1}^{n} \frac{|s_i - \text{med}(s)|}{\text{med}(s)}
\end{equation}

Stress consistency ($R_{\text{stress}}$) compares consecutive line stress sequences. The final score combines both: $\text{RSFS} = 0.7 R_s + 0.3 R_{\text{stress}}$, prioritizing syllable uniformity while acknowledging stress patterns.

\paragraph{EAS: Emotion Arc Score.}
EAS measures emotional continuity and coherence across the lyric sequence. For each line $i$, the GoEmotions classifier (mapped to 6 FER classes) produces normalized emotion vector $\mathbf{q}_i \in [0,1]^6$. Transitions between consecutive lines use Jensen-Shannon divergence:

\begin{equation}
d_i = \frac{1}{2}\left[\text{KL}(\mathbf{q}_i \| \mathbf{m}) + \text{KL}(\mathbf{q}_{i+1} \| \mathbf{m})\right]
\end{equation}

where $\mathbf{m} = \frac{1}{2}(\mathbf{q}_i + \mathbf{q}_{i+1})$. Each transition is weighted by emotional intensity $w_i = 1 - q_i^{\text{neutral}}$, downweighting neutral lines. Final score: $\text{EAS} = \exp(-\bar{d}_w)$.

\paragraph{HQCS: Hook Quality and Catchiness Score.}
HQCS identifies and rewards repeated $n$-gram patterns (hooks) that enhance memorability and catchiness. Hooks are linguistic patterns listeners recognize and remember (e.g., recurring refrains). We extract all 3-grams and count those appearing multiple times. HQCS combines three metrics:

\begin{equation}
\text{HQCS} = 0.4 \cdot \text{sal} + 0.3 \cdot \text{cov} + 0.3 \cdot \text{flow}
\end{equation}

where salience = max hook frequency / total $n$-grams, coverage = sum hook frequencies / total $n$-grams, and flow = mean(PPFS, RSFS). A diversity penalty balances hook repetition against monotony.

\paragraph{RCS: Rhyme Consistency Score.}
RCS evaluates rhyme quality by comparing phonetic endings of consecutive line-final words. Rhyme quality is determined from the last stressed vowel onward using CMU Pronouncing Dictionary. Similarity between consecutive rhyme patterns is computed by matching phonemes backward from the end. RCS is the mean similarity across all consecutive line pairs.

\paragraph{MCS: Motif Consistency Score.}
MCS measures semantic coherence and thematic consistency using pre-trained sentence embeddings. High-quality lyrics maintain topical coherence across lines. MCS balances local and global consistency:

\begin{equation}
\text{MCS} = 0.7 \cdot L + 0.3 \cdot G
\end{equation}

where $L$ = mean cosine similarity of consecutive lines (local coherence), and $G$ = mean similarity of sampled non-adjacent pairs (global thematic consistency).

\paragraph{DPS: Degeneracy Penalty Score.}
DPS penalizes excessive lexical repetition, indicating degenerate output lacking diversity. The penalty is:

\begin{equation}
\text{DPS} = \exp(-3.0 \cdot r)
\end{equation}

where $r = \frac{\max \text{ word frequency}}{\text{total words}}$. Low repetition yields DPS $\approx 1.0$; high repetition decays exponentially. This rewards lexical diversity while allowing natural reuse.

\paragraph{LPS: Length Penalty Score.}
LPS ensures generated lyrics have sufficient length and substance. Very short lyrics often lack narrative depth or emotional development. The penalty is:

\begin{equation}
\text{LPS} = \begin{cases} 1.0 & \text{if } n \geq 8 \\ \left(\frac{n}{8}\right)^{2.0} & \text{otherwise} \end{cases}
\end{equation}

where $n$ is the number of lines. Lyrics with 8 or more lines receive no penalty. Shorter lyrics face exponential penalty (exponent 2.0), strongly discouraging very brief outputs while allowing some flexibility for shorter compositions. The final PHREM score is adjusted via multiplication:

\begin{equation}
\text{PHREM}_{\text{final}} = \text{PHREM} \times \text{LPS}
\end{equation}

All components operate on $[0,1]$ scales, making PHREM directly interpretable as a lyric quality score. The weighted combination reflects relative importance: emotional control (EAS, 0.22) and semantic coherence (MCS, 0.20) dominate overall assessment.
...
as effectively as larger autoregressive models.  The PHREM
...
the PHREM composite evaluation metric.
